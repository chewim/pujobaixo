...
Como esta sección vive sobre la misma foto de fondo fija (no sobre `#fff`), cada bloque de texto lleva la clase `.pj-glass`: fondo blanco al 82% + `backdrop-filter: blur(20px)` + borde sutil `rgba(255,255,255,0.6)` — efecto "cristal esmerilado" tipo Revolut, en islas independientes (una por bloque, no un overlay único para toda la sección). **Importante de accesibilidad**: con el blur, el texto secundario gris estándar (`#6B6F7B`) puede bajar de 4.5:1 si el cristal cae sobre una zona oscura de la ilustración — dentro de `.pj-glass` usa siempre `#4B4E58` (mínimo 5.44:1 incluso en el peor caso, fondo negro sólido detrás) en vez de `#6B6F7B` para párrafos. El texto principal (`#14151A`) no tiene este problema (>11:1 en cualquier caso). Si un navegador no soporta `backdrop-filter`, `@supports` sube la opacidad a 94% para no perder legibilidad.
