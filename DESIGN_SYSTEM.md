...
- Contenedor "hero" en degradado lavanda: `radial-gradient(120% 90% at 0% 0%, rgba(255,255,255,0.5), transparent 55%)` sobre `linear-gradient(135deg, #A79DFF 0%, #8F83FC 45%, #7D6EF7 100%)`, radio 16, sin borde, sombra `0 12px 32px -20px rgba(91,76,251,0.4)`. El radial es el brillo superior-izquierdo. Se eligió un tono intermedio a propósito: la primera versión en morado pleno empataba en peso con el botón Publicar contiguo, y el CTA debe ganar siempre. Nota de contraste: blanco sobre este lavanda ronda 3:1 (por debajo de AA); asumido conscientemente por jerarquía — si se prioriza accesibilidad, oscurecer un punto el degradado o pasar el texto a morado muy oscuro. Contenido en blanco: título `#fff`, subtítulo `rgba(255,255,255,0.78)`, icono en círculo `rgba(255,255,255,0.16)`, "Canviar" blanco subrayado, focus en blanco. Interruptor invertido: pista apagada `rgba(255,255,255,0.28)` con botón blanco; activa pista blanca con botón `#5B4CFB`. Los segmentados interiores se quedan blancos: son la superficie de control dentro del objeto.
- Es el **único** componente con degradado y sombra de color del sistema; su función es señalar el beneficio destacado del formulario. No extender a otros componentes sin decisión explícita — si todo brilla, nada brilla.
- Pill "Nou" (`.pj-benefit-new`): sobre el título, invertida (fondo blanco, texto `#5B4CFB`), 10.5px/800 mayúsculas con tracking. Temporal: retirar cuando la funcionalidad deje de ser nueva (~1 mes tras el lanzamiento, sept-oct 2026).
