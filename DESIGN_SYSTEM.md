...
Solo en `isLoginMode`, debajo de la tarjeta de acceso (nunca en signup/forgot/reset, para no distraer esos flujos): un titular corto (h2, 26px/800) + subtítulo, seguido de bloques `.pj-value-block` — imagen (esquinas 24px) + h2 (22px/800) + párrafo (15px). Apilado y centrado en móvil; en desktop (≥900px) pasa a fila con imagen+texto alternados (`.pj-value-block-reverse` invierte el orden) y texto alineado a la izquierda. Referencia de estilo: Cabify/Uber/Revolut/BlaBlaCar — frases cortas y concretas, mucho espacio en blanco, una sola imagen por bloque, nada de iconos decorativos extra.

**Sistema de 3 pilares** (patrón clásico de landing de marketplace tipo Cabify/BlaBlaCar: económico + social + confianza), en orden normal/reverse/normal:
1. **Estalvi** (`auth-savings.png`) — "Comparteix les despeses, minimitza el teu gasto".
2. **Comunitat** (`auth-community.png`) — "No són desconeguts, són veïns".
3. **Confiança** (mockup SVG, no foto — ver abajo) — "Coneix qui condueix abans de pujar". Ataca la objeción psicológica principal del carpooling ("viajar con desconocidos"), igual que el pilar de seguridad de Cabify o el de confianza/perfiles de BlaBlaCar — pero con copy honesto a lo que la app realmente ofrece (nombre, teléfono y WhatsApp visibles antes de confirmar), no verificación de identidad ni valoraciones que no existen en el producto.

**El tercer pilar usa un mockup ilustrado de la UI real (`<svg class="pj-value-mockup">` inline), no una foto de stock ni una captura real.** Es un "teléfono" dibujado a mano (marco `#14151A`, pantalla `#F3F4F8`) con dos tarjetas de viaje que reutilizan exactamente los mismos componentes visuales de la app real: pill de ruta (`#EEEBFF`/`#5B4CFB`), avatar circular, botón de WhatsApp con el contraste ya corregido (fondo `#25D366`, texto oscuro `#14151A` — el mismo fix documentado más abajo en "Patrones de interacción"), pill de precio morada, y la bottom nav con "Viatges" activo. Se eligió SVG inline en vez de un PNG porque escala nítido a cualquier tamaño sin pesar un asset extra, y porque es fácil mantenerlo sincronizado si los componentes reales cambian de color. Medidas vía clase `.pj-value-mockup` (`max-width:200px` móvil, `260px` desktop), paralela a la regla `.pj-value-block img` pero sin `border-radius` (el marco del teléfono ya lleva las esquinas redondeadas dibujadas).

**Línea de alcance antes del footer**: `<p>` centrado, `#5B4CFB`, 700, entre el tercer bloque de valor y el CTA final — "De Berga, Gironella i tot el Berguedà, cap a Barcelona — i tornada". Cumple la función del "dato de escala" que usa Cabify ("6 países, 40 ciudades") pero sin inventarse ninguna cifra de uso real: ancla en lugares geográficos concretos y verificables, no en un contador de viajes/usuarios que ahora mismo no tenemos forma de mostrar con honestidad.

**La foto de fondo es fija (parallax), igual en los 4 modos (login/signup/forgot/reset).** El contenedor de la imagen usa `position:fixed; inset:0; z-index:0`, así que no se mueve con el scroll — la tarjeta de acceso flota por encima (`z-index:1`) y dejas ver la foto alrededor. No hay degradado ni recorte de altura: la sensación de "cristal" o "corte" no se resuelve difuminando la imagen, sino tapándola con contenido opaco.

Justo después de la tarjeta (solo en `isLoginMode`), el bloque de valor vive dentro de un contenedor **full-bleed** con fondo sólido `#F3F4F8` (el mismo que `html, body`): `width:100vw; margin: 40px calc(50% - 50vw) 0;` — este truco saca el fondo del ancho limitado de `.pj-main` (max-width:920px) para que cubra todo el viewport de lado a lado, no solo la columna central. Como es opaco y arranca justo debajo de la tarjeta, tapa la foto fija por completo en cuanto el usuario hace scroll — ahí es donde ocurre el "corte" limpio, no en la imagen. `html, body` llevan `overflow-x:hidden` para evitar el scroll horizontal de unos pocos píxeles que el truco `100vw` puede introducir en navegadores con scrollbar no superpuesta (Windows/desktop). Nada de cristal esmerilado ni `backdrop-filter`; los párrafos de `.pj-value-block-copy` usan el gris estándar `#6B6F7B` (>4.5:1 de sobra sobre `#F3F4F8`).

**En desktop (≥900px) la tarjeta ocupa la pantalla completa antes de que aparezca el resto de la información.** El div que envuelve la tarjeta (`.pj-auth-first-screen`) no tiene estilo propio en móvil (mismo comportamiento de siempre), pero a partir de 900px gana `min-height:100vh; display:flex; align-items:center; justify-content:center` — convierte esa primera pantalla en un hero de portada real (imagen a pantalla completa + tarjeta centrada), y el bloque de valor solo aparece al hacer scroll. En móvil no se aplica esto a propósito: con pantallas más cortas, forzar 100vh dejaría un hueco vacío incómodo entre la tarjeta y la info.

**`object-position` es distinto en móvil y desktop** (clase `.pj-auth-hero-img`, override en el mismo `@media (min-width:900px)`): en móvil `15% 35%` (coche centrado en la franja horizontal recortada, ver más arriba); en desktop `15% 55%` — con la tarjeta ahora ocupando toda la pantalla (`min-height:100vh`), muchos portátiles recortan en vertical, y con el punto de enfoque más bajo se prioriza ver el coche (parte baja de la ilustración) sacrificando algo de cielo/montaña arriba si hace falta recortar. El override usa `!important` porque el estilo base va inline en el `<img>` (mayor especificidad que una clase normal).

**Footer de cierre del bloque de valor**: es su propia sección full-bleed (mismo truco `width:100vw; margin:0 calc(50% - 50vw)`), separada de los `.pj-value-block` a propósito — fondo sólido `#5B4CFB` (morado de marca) en vez de compartir el gris `#F3F4F8` de arriba, para que se lea como un cierre diferenciado y no como una continuación más del bloque de valor. Contraste verificado: titular blanco 5.42:1, subtítulo `rgba(255,255,255,0.92)` 4.7:1 (con menos opacidad cae por debajo del mínimo AA de 4.5:1), botón invertido (fondo blanco, texto `#5B4CFB`) 5.42:1. El botón ejecuta `scrollToLoginCard` (`window.scrollTo({top:0, behavior:'smooth'})`) — vuelve a la tarjeta de acceso en vez de navegar a otra vista, ya que el bloque de valor solo existe dentro de `isLoginMode`, sobre la misma pantalla.

Tiene **su propio layout** (clases `.pj-footer-cta`/`.pj-footer-cta-copy`/`.pj-footer-cta-btn`, no reutiliza `.pj-value-block`): en móvil apilado y centrado (`max-width:420px`), en desktop (≥900px) pasa a fila — texto a la izquierda, botón a la derecha (`justify-content:space-between`, `max-width:840px` a juego con los bloques de valor de arriba). Es una estructura deliberadamente distinta a la del titular introductorio (centrado) y a la de los `.pj-value-block` (imagen+texto), para reforzar que es un bloque de cierre con identidad propia.

**Altura del footer: fija, no estirada.** Se probó a hacerlo crecer con `flex:1` dentro de un wrapper `min-height:100vh` para que siempre tocara el final del viewport, pero en pantallas altas con poco contenido arriba quedaba desproporcionadamente alto — no es el patrón habitual de un footer web (que se dimensiona por su contenido + padding, no por el alto de la pantalla). Se revirtió: `.pj-footer-cta` vuelve a tener solo su padding fijo (`64px 16px`), sin `flex-grow` ni wrapper especial. Como consecuencia, en pantallas muy altas con contenido corto puede quedar una franja mínima del fondo de página después del footer — es el comportamiento normal de cualquier footer no-sticky, no un bug.

**Ojo con `.pj-main` y su padding inferior de 104px** (`padding: 24px 16px 104px` móvil / `32px 24px 104px` desktop, pensado para dejar hueco a la bottom nav). Ese padding vive en `.pj-main`, **fuera** de la caja del `min-height:100vh` de arriba, así que aunque el footer llegue a cubrir el 100vh, esos 104px de después seguían quedando sin nada opaco encima — dejando ver la foto fija de fondo justo al final del scroll. Se corrige con `mainStyle` (computado en JS): cuando `isAuthShellView` es true, `.pj-main` recibe `padding-bottom:0` inline (pisa el `104px` de la clase, que de todos modos es para la bottom nav — ausente siempre en la pantalla de auth, ya que `showBottomNav` requiere `isLoggedIn`). El padding superior no se toca, solo el inferior es el que dejaba el hueco.

`object-position` de la imagen es `15% 35%` (no `center`): en `object-fit:cover`, en pantallas estrechas el recorte es horizontal, y con el punto de foco centrado la franja visible caía en la zona vacía de la ilustración (entre el coche y la Sagrada Família, sin mostrar ninguno de los dos). Desplazando el foco a la izquierda, el coche queda visible junto a la tarjeta en composición con el layout del formulario. En desktop el recorte es sobre todo vertical, así que este ajuste apenas cambia el encuadre ahí.
