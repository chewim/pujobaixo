...
**Footer de cierre del bloque de valor**: tras los dos `.pj-value-block`, dentro del mismo contenedor full-bleed `#F3F4F8`, un bloque centrado (`max-width:420px`, separado con `border-top:1px solid #E4E5EC`) con un titular corto de prueba social/urgencia + un botón primario "Inicia sessió" que ejecuta `scrollToLoginCard` (`window.scrollTo({top:0, behavior:'smooth'})`) — vuelve a la tarjeta de acceso en vez de navegar a otra vista, ya que el bloque de valor solo existe dentro de `isLoginMode`, sobre la misma pantalla.
