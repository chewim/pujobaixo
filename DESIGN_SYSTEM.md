...
Solo en `isLoginMode`, debajo de la tarjeta de acceso (nunca en signup/forgot/reset, para no distraer esos flujos): un titular corto (h2, 26px/800) + subtítulo, seguido de bloques `.pj-value-block` — imagen (esquinas 24px) + h2 (22px/800) + párrafo (15px). Apilado y centrado en móvil; en desktop (≥900px) pasa a fila con imagen+texto alternados (`.pj-value-block-reverse` invierte el orden) y texto alineado a la izquierda. Referencia de estilo: Cabify/Uber/Revolut — frases cortas y concretas, mucho espacio en blanco, una sola imagen por bloque, nada de iconos decorativos extra.
...
**El footer llega siempre hasta el final de la página**, incluso si el contenido (tarjeta + bloques de valor) no llena la pantalla. Solo en `isLoginMode`, el wrapper raíz de todo el auth shell (`authShellWrapStyle`, computado en JS) pasa de `position:relative;` a `position:relative; display:flex; flex-direction:column; min-height:100vh;` — convierte la tarjeta, el bloque de valor y el footer en hijos flex de una columna con al menos una pantalla de alto. El footer añade `flex:1 0 auto;` (crece para ocupar el espacio sobrante, nunca se encoge por debajo de su contenido) y `display:flex; align-items:center;` para centrar verticalmente `.pj-footer-cta` si el bloque queda más alto de lo necesario. En signup/forgot/reset no se activa (siguen con `position:relative;` simple), y en login con contenido ya más alto que una pantalla no cambia nada — el `flex:1` solo actúa cuando sobra espacio.
