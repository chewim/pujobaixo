...
**Footer de cierre del bloque de valor**: es su propia sección full-bleed (mismo truco `width:100vw; margin:0 calc(50% - 50vw)`), separada de los `.pj-value-block` a propósito — fondo sólido `#5B4CFB` (morado de marca) en vez de compartir el gris `#F3F4F8` de arriba, para que se lea como un cierre diferenciado y no como una continuación más del bloque de valor. Contenido centrado (`max-width:420px`): titular corto de prueba social/urgencia en blanco, subtítulo en `rgba(255,255,255,0.92)` (4.7:1 sobre el morado, por encima del mínimo AA de 4.5:1 — con menos opacidad cae por debajo, verificado), y un botón invertido (fondo blanco, texto `#5B4CFB`, 5.42:1) que ejecuta `scrollToLoginCard` (`window.scrollTo({top:0, behavior:'smooth'})`) — vuelve a la tarjeta de acceso en vez de navegar a otra vista, ya que el bloque de valor solo existe dentro de `isLoginMode`, sobre la misma pantalla.
