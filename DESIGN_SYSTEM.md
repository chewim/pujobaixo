...
**La foto de fondo está acotada, no es fija a toda la página.** Solo en `isLoginMode` el contenedor de la imagen (`heroContainerStyle`) pasa de `position:fixed; inset:0` (comportamiento normal en signup/forgot/reset, cubre todo el viewport) a `position:absolute; top:0; height:760px`, es decir, vive dentro del flujo normal y se corta a los 760px. Dentro de ese contenedor, los últimos 240px llevan un degradado (`rgba(243,244,248,0)` → `#F3F4F8`) que difumina la imagen hacia el fondo plano de la página. Como la tarjeta de login + los bloques de valor son más altos que 760px, el degradado hace la transición y el resto del bloque de valor cae sobre `#F3F4F8` liso — nada de cristal esmerilado ni `backdrop-filter`. Por eso los párrafos de `.pj-value-block-copy` usan el gris estándar `#6B6F7B` (>4.5:1 de sobra sobre fondo blanco/`#F3F4F8`), sin necesidad de la variante oscura que exigía el efecto glass.

`object-position` de la imagen es `15% 35%` (no `center`): en `object-fit:cover`, en pantallas estrechas el recorte es horizontal, y con el punto de foco centrado la franja visible caía en la zona vacía de la ilustración (entre el coche y la Sagrada Família, sin mostrar ninguno de los dos). Desplazando el foco a la izquierda, el coche queda centrado en la parte baja del hero — justo la franja que se ve debajo de la tarjeta antes del degradado — en composición con el layout del formulario. En desktop el recorte es sobre todo vertical, así que este ajuste apenas cambia el encuadre ahí.
